...
const ROLE_STYLE: Record<UserRole, string> = {
  ...
  project_manager: "bg-violet-500/15 text-violet-400",
  member:          "bg-[rgba(60,63,68,0.5)] text-[#ABAEBB]",
};
    ...
    >
      <div className="bg-[#0E111B] rounded-2xl shadow-2xl w-full max-w-sm p-7 border border-[rgba(60,63,68,0.5)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#7B8299] mb-1.5">Rolle</label>
            <select
              value={role}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-[#2862D7] text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-[#3470E8] disabled:opacity-50 transition-colors"
            >
...
function MembersPanel({
  ...
  return (
    <div
      ...
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="bg-[#0E111B] h-full w-full max-w-sm shadow-2xl p-6 overflow-y-auto flex flex-col gap-5 border-l border-[rgba(60,63,68,0.5)]">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest">Mitglieder</p>
            <h3 className="text-base font-bold text-white mt-0.5">{project.name}</h3>
          </div>
          <button onClick={onClose} className="text-[#7B8299] hover:text-white transition-colors">
        ...
        {available.length > 0 && (
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 rounded-xl px-3 py-2 text-sm text-white bg-[rgba(23,37,64,0.6)] border border-[rgba(133,166,233,0.25)] focus:outline-none focus:ring-2 focus:ring-[#2862D7]/40 focus:border-[#2862D7] transition-colors appearance-none"
            >
            ...
            <button
              ...
              className="px-4 py-2 bg-[#2862D7] text-white text-sm font-semibold rounded-xl hover:bg-[#3470E8] disabled:opacity-40 transition-colors"
            >
              +
            </button>
          </div>
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2].map(i => <div key={i} className="h-12 bg-[#172540] rounded-xl animate-pulse" />)}
          </div>
        ) : members.length === 0 ? (
          <p className="text-sm text-[#7B8299] text-center py-8">Keine Mitglieder zugewiesen</p>
        ) : (
          <div className="space-y-2">
            {members.map(m => (
              <div key={m.user_id} className="flex items-center justify-between bg-[#172540] rounded-xl px-4 py-3 border border-[rgba(60,63,68,0.5)]">
                <div>
                  <p className="text-sm font-medium text-white">{m.users?.email ?? m.user_id}</p>
                  <RoleBadge role={(m.users?.role ?? "member") as UserRole} />
                </div>
                <button
                  onClick={() => removeMember(m.user_id)}
                  className="text-[#7B8299] hover:text-red-400 transition-colors ml-3"
                  title="Entfernen"
                >
...
function Toast({ message, onDone }: { message: string; onDone: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] bg-[#172540] border border-[rgba(60,63,68,0.5)] text-white text-sm px-5 py-3 rounded-2xl shadow-2xl animate-in slide-in-from-bottom-2">
      {message}
    </div>
  );
}
...
export default function OrgAdminPage() {
  ...
  return (
    <>
      ...
      {/* Header */}
      <header className="bg-[#0E111B] border-b border-[rgba(60,63,68,0.5)] sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push("/dashboard")}
              className="text-[#7B8299] hover:text-white transition-colors"
              title="Zum Dashboard"
            >
            ...
            <span className="text-sm font-semibold text-white">Organisation verwalten</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest hidden sm:block">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-[rgba(23,37,64,0.6)] rounded-xl p-1 w-fit">
        ...
        {tab === "projects" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-white">Alle Projekte</h2>
            </div>

            {loading ? (
              <div className="space-y-2">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-16 bg-[#172540] border border-[rgba(60,63,68,0.5)] rounded-2xl animate-pulse" />
                ))}
              </div>
            ) : projects.length === 0 ? (
              <div className="bg-[#172540] border border-[rgba(60,63,68,0.5)] rounded-2xl p-12 text-center">
                <p className="text-[#7B8299] text-sm">Noch keine Projekte</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm min-w-[720px]">
                  <thead>
                    <tr className="border-b border-[rgba(60,63,68,0.4)] bg-[rgba(23,37,64,0.5)]">
                      <th className="text-left text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest px-5 py-3">Projekt</th>
                      <th className="text-left text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest px-5 py-3 hidden sm:table-cell">Ort</th>
                      <th className="text-left text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest px-5 py-3 hidden md:table-cell">Pläne</th>
                      <th className="text-left text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest px-5 py-3 hidden md:table-cell">Mitglieder</th>
                      <th className="text-left text-[10px] font-semibold text-[#7B8299] uppercase tracking-widest px-5 py-3 hidden lg:table-cell">Erstellt</th>
                      <th className="px-5 py-3" />
                    </tr>
                  </thead>
                  <tbody>
                    {projects.map((p, i) => (
                      <tr key={p.id} className={`hover:bg-[#1E2D4A]/60 transition-colors ${i < projects.length - 1 ? "border-b border-[rgba(60,63,68,0.3)]" : ""}`}>
                        <td className="px-5 py-3.5">
                          <button
                            onClick={() => router.push(`/projects/${p.id}/norms`)}
                            className="font-semibold text-white hover:text-[#85A6E9] transition-colors text-left"
                          >
                            {p.name}
                          </button>
                          {p.bauzone && (
                            <p className="text-[10px] text-[#7B8299] mt-0.5">{p.bauzone}</p>
                          )}
                        </td>
                        <td className="px-5 py-3.5 text-[#ABAEBB] hidden sm:table-cell">
                          {p.location?.municipality ? `${p.location.municipality}, ${p.location.canton ?? ""}` : "—"}
                        </td>
                        <td className="px-5 py-3.5 text-[#ABAEBB] hidden md:table-cell">
                          {p.documents?.[0]?.count ?? 0}
                        </td>
                        <td className="px-5 py-3.5 hidden md:table-cell">
                          <button
                            onClick={() => setMembersProject(p)}
                            className="text-[#ABAEBB] hover:text-[#85A6E9] transition-colors flex items-center gap-1.5"
                          >
                        ...
                        <td className="px-5 py-3.5 text-[#7B8299] text-xs hidden lg:table-cell">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2 justify-end">
                            <button
                              onClick={() => setMembersProject(p)}
                              className="text-xs font-medium text-[#7B8299] hover:text-[#85A6E9] transition-colors px-2 py-1 rounded-lg hover:bg-[#2862D7]/10"
                            >
                              Mitglieder
                            </button>
                            <button
                              onClick={() => setDeleteProject(p)}
                              className="text-[#7B8299] hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-red-500/10"
                              title="Projekt löschen"
                            >
        ...
        {tab === "members" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-white">Mitglieder der Organisation</h2>
              <button
                onClick={() => setInviteOpen(true)}
                className="flex items-center gap-1.5 bg-[#2862D7] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-[#3470E8] active:scale-[0.97] transition-all shadow-sm shadow-[#2862D7]/25"
              >
            ...
            {loading ? (
              <div className="space-y-2">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-14 bg-[#172540] border border-[rgba(60,63,68,0.5)] rounded-2xl animate-pulse" />
                ))}
              </div>
            ) : users.length === 0 ? (
              <div className="bg-[#172540] border border-[rgba(60,63,68,0.5)] rounded-2xl p-12 text-center">
                <p className="text-[#7B8299] text-sm">Noch keine Mitglieder</p>
              </div>
            ) : (
              <div className="bg-[#172540] border border-[rgba(60,63,68,0.5)] rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
                      ...
                      <tr key={u.id} className={`hover:bg-[#1E2D4A]/60 transition-colors ${i < users.length - 1 ? "border-b border-[rgba(60,63,68,0.3)]" : ""}`}>
                        ...
                        <td className="px-5 py-3.5">
                          <select
                            ...
                            className="border border-[rgba(133,166,233,0.25)] rounded-lg px-2 py-1.5 text-xs bg-[rgba(23,37,64,0.6)] text-white focus:outline-none focus:ring-2 focus:ring-[#2862D7]/30 focus:border-[#2862D7] transition-colors appearance-none"
                          >
                            {ASSIGNABLE_ROLES.map(r => (
                              <option key={r} value={r}>{ROLE_LABELS[r]}</option>
                            ))}
                        ...
                        <td className="px-5 py-3.5 text-[#7B8299] text-xs hidden sm:table-cell">
