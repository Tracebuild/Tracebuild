    ...
    <section id="story" ref={storyRef} style={{ position: "relative", height: "440vh" }}>
      <div style={{ ...pinStyle, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: "100px 24px 20px" }}>
        ...
        <div style={{
          ...
          background: "#0E111B",
          border: "1px solid rgba(60,63,68,0.5)",
          borderRadius: 12,
          ...
          <div style={{
            ...
            background: "#172540",
            borderRight: "1px solid rgba(60,63,68,0.4)",
            padding: "18px 14px",
            ...
            {NAV_ITEMS.map((item, i) => (
              <div key={item} style={{
                ...
                ...(i === 2
                  ? { background: "rgba(40,98,215,0.1)", color: "#2862D7" }
                  : { color: "#7B8299" }),
              }}>{item}</div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{ padding: "14px 18px", borderBottom: "1px solid rgba(60,63,68,0.4)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 12.5, color: "#FFFFFF", fontWeight: 500 }}>Grundriss_EG.pdf</div>
                <div style={{ fontSize: 9, color: "#6B7086", marginTop: 2 }}>Hochgeladen · 10:32</div>
              </div>
              <div style={{
                background: "#2862D7", color: "#0B0C0E", borderRadius: 6,
                padding: "6px 12px", fontSize: 10, fontWeight: 600,
                boxShadow: `0 0 0 ${(reportGlow * 5).toFixed(1)}px rgba(40,98,215,${(reportGlow * 0.35).toFixed(2)})`,
            ...
            <div style={{ flex: 1, position: "relative", padding: 16 }}>
              <svg viewBox="0 0 380 250" style={{ width: "100%", height: "100%", display: "block" }}>
                <g style={{ opacity: drawT, transition: `opacity .5s ${EASE}` }}>
                  <rect x="10" y="10" width="360" height="230" fill="none" stroke="rgba(133,166,233,0.25)" strokeWidth="1" />
                  <rect x="170" y="10"  width="4" height="100" fill="rgba(133,166,233,0.4)" />
                  <rect x="170" y="140" width="4" height="100" fill="rgba(133,166,233,0.4)" />
                  <rect x="10"  y="128" width="100" height="4" fill="rgba(133,166,233,0.4)" />
                  <rect x="142" y="128" width="228" height="4" fill="rgba(133,166,233,0.4)" />
                  <rect x="250" y="128" width="4"   height="112" fill="rgba(133,166,233,0.4)" />
                  <text x="35"  y="70"  fill="rgba(133,166,233,0.6)" fontSize="10">Zimmer 1</text>
                  <text x="195" y="70"  fill="rgba(133,166,233,0.6)" fontSize="10">Zimmer 2</text>
                  <text x="35"  y="190" fill="rgba(133,166,233,0.6)" fontSize="10">Wohnen / Essen</text>
                  <text x="280" y="190" fill="rgba(133,166,233,0.6)" fontSize="9">Bad</text>
                </g>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, color: "#FFFFFF", fontWeight: 500 }}>Analyseübersicht</div>
            ...
            <div style={{
              ...
              background: "rgba(40,98,215,0.06)", borderRadius: 8, padding: 10,
            }}>
              <svg viewBox="0 0 36 36" style={{ width: 38, height: 38, flexShrink: 0 }}>
                <circle cx="18" cy="18" r="15" fill="none" stroke="rgba(133,166,233,0.15)" strokeWidth="3" />
                <circle cx="18" cy="18" r="15" fill="none" stroke="#2862D7" strokeWidth="3"
                  strokeDasharray="94.2" strokeDashoffset={donutOffset}
                  style={{ transition: "stroke-dashoffset 0.3s linear" }}
                  transform="rotate(-90 18 18)" />
              </svg>
              <div>
                <div style={{ fontSize: 13, color: "#FFFFFF", fontWeight: 600 }}>72%</div>
            ...
            {FINDINGS.map(({ label, severity, color, desc, at }) => (
              <div key={label} style={{
                ...
                background: "rgba(40,98,215,0.05)", borderRadius: 6, padding: 8,
              }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 10, color: "#FFFFFF" }}>{label}</span>
                  <span style={{ fontSize: 8, color }}>{severity}</span>
                </div>
                <span style={{ fontSize: 8.5, color: "#6B7086" }}>{desc}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop: 26, display: "flex", gap: 8 }}>
          {[0, 1, 2, 3].map(i => (
            <div key={i} style={{
              ...
              background: beatIdx === i ? "#2862D7" : "rgba(133,166,233,0.25)",
